...
function deploy_elassandra_operator() {
  helm install --name strapkop --namespace $NAMESPACE \
    ...
    --set dns.azure.secretName= \
    /home/eric/strapdata/dev/strapkop/helm/build/helm/charts/elassandra-operator-0.3.0-rc1.tgz

}

function deploy_elassandra_datacenter() {
  helm install --name ${CLUSTER_NAME}-${DATACENTER_NAME} --namespace $NAMESPACE --set userConfigMapVolumeSource.name=elassandra-${CLUSTER_NAME}-${DATACENTER_NAME}-user-config -f values-datacenter-thingsboard.yml /home/eric/strapdata/dev/strapkop/helm/build/helm/charts/elassandra-datacenter-0.3.0-rc1.tgz
...
function create_keyspace() {
  ...
  kubectl exec -n $NAMESPACE -it ${ELASSANDRA_NODE:4} -- source /usr/share/cassandra/aliases.sh && cqlsh -e "CREATE KEYSPACE IF NOT EXISTS thingsboard WITH replication = { 'class' : 'NetworkTopologyStrategy', 'dc1' : '1' };"
}
